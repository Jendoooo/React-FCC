...
function Joke(props){
    return(
        <div className="cc">
            ...
            <h2 style={{display: props.qanda.question ? "block" :"none"}}
            >question: {props.qanda.question}</h2>
           {/* here we are telling the code to render a blue reponse for 
           the comments without questions */}
            <h3 style={{color: !props.qanda.question && "blue"}}>
                response: {props.qanda.punchline}</h3>
        </div>
    )
}
